...
void GetMaxCoors(int nQuaternions, int nAtoms, float *coors, float* maxcoors)
{
  ...
  for (int i = 0; i < nQuaternions; i++)
  {
    ...
    {
      if (coors[(i*nAtoms+k)*3 + 0] > maxcoors[i*3 + 0])
      {
	maxcoors[i*3 + 0] = coors[(i*nAtoms+k)*3 + 0];
      }
      if (coors[(i*nAtoms+k)*3 + 1] > maxcoors[i*3 + 1])
      {
	maxcoors[i*3 + 1] = coors[(i*nAtoms+k)*3 + 1];
      }
      if (coors[(i*nAtoms+k)*3 + 2] > maxcoors[i*3 + 2])
      {
	maxcoors[i*3 + 2] = coors[(i*nAtoms+k)*3 + 2];
      }
    }
  }
}
